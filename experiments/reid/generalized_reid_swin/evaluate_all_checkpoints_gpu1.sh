...
EVAL_DIR=$EXP/evaluate/all_checkpoints
...
evaluate_one() {
  ...
  local gallery=$4
  local num_query=$5
  local checkpoint_name log_file sanitized exit_code status
...
evaluate_checkpoint() {
  local checkpoint=$1
  local checkpoint_name sanitized
  checkpoint_name=$(basename "$checkpoint" .pth)
  sanitized=$(dirname "$checkpoint")/sanitized_$(basename "$checkpoint")
  ...
  evaluate_one prcc "$checkpoint" \
    ...
    71
  evaluate_one combined_stress "$checkpoint" \
    "$EXP/data/official_stress/query" \
    "$EXP/data/official_stress/bounding_box_test" \
    1266

  cleanup_sanitized "$sanitized"
  echo "Finished all targets: $checkpoint_name"
}
...
echo "Generalized multi-target checkpoint evaluation started."
echo "Host GPU: $GPU_DEVICE"
echo "Re-ranking: $RE_RANKING"
echo "Stable checkpoints currently available: ${#checkpoints[@]}"
...
echo "Generalized multi-target checkpoint evaluation complete."
cat "$SUMMARY"
