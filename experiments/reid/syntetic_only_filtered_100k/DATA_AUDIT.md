# Synthetic-Only Filtered 100k Data Audit
...
The original manifest contains `233840` crops in `2054` person-at-moment groups across `39` identities. The exact `100000`-crop training set keeps up to `48` lowest-ID variants from every group, then adds the next variant from `1644` groups. Extra groups are selected deterministically in identity-balanced round-robin order.
