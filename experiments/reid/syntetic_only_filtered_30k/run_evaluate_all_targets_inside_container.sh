...
CONFIG=$EXP/configs/${EXPERIMENT_NAME}.yaml
TRAIN_DIR=$EXP/results/${EXPERIMENT_NAME}/train
...
evaluate_one() {
  ...
  if [[ ! -f "$sanitized" ]]; then
    ...
    python3 "$SANITIZE" "$checkpoint" "$sanitized" >/dev/null
  fi
...
echo "Synthetic-only filtered 30k all-target raw evaluation started inside TAO container."
